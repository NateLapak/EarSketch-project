...
#
#		author: Nathan Lapak
#		description: My entry for the eark sketch competition. Song only runs on the website with the 
#       dedicated libraries installed
#
...
def chorus():
    ...
    fitMedia(ENTREP_VOX_PHARRELL_VRS_1, 5, 27, 35.4)
    
    setEffect(1, VOLUME, GAIN, 0, 26, -15, 28)
    setEffect(2, VOLUME, GAIN, 0, 26, -15, 28)
    setEffect(3, VOLUME, GAIN, 0, 26, -15, 28)
    
    # For loop to repeat the same instruments over unique measures
    for measure in range(30, 32):
        
        # Repeat only in measures that are even
        if measure % 2 == 0:
            fitMedia(ENTREP_VOX_PHARRELL_VRS_1, 5, measure, measure + 1)
    ...
    setEffect(3, VOLUME, GAIN, -5, 34, 0, 38)
    setEffect(2, VOLUME, GAIN, -5, 34, 0, 38)
    setEffect(1, VOLUME, GAIN, -5, 38, 0, 41)
